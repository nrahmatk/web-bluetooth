...
document.addEventListener('DOMContentLoaded', function() {
    ...
    log('Aplikasi siap digunakan. Pastikan Bluetooth aktif dan printer dalam mode pairing.', 'info');
});
...
async function printReceipt() {
    ...
    try {
        ...
        // Header
        await printer.sendData(escpos.generateReceiptHeader(storeName, storeAddress, cashier));
        ...
        // Footer
        await printer.sendData(escpos.generateReceiptFooter(total, paymentAmount, paymentMethod));
...
async function testBasicPrint() {
    ...
    try {
        await printer.sendData(escpos.init());
        await printer.sendData(escpos.printText('TEST BASIC PRINT', { align: 'center', bold: true }));
        await printer.sendData(escpos.printText('Printer: EPSON EP5821'));
        await printer.sendData(escpos.printText('Lebar: 58mm'));
        await printer.sendData(escpos.printText(`Tanggal: ${escpos.formatDateTime()}`));
        await printer.sendData(escpos.printSeparator('='));
        await printer.sendData(escpos.printText('Test berhasil!', { align: 'center' }));
        await printer.sendData(escpos.feedLines(3));
        await printer.sendData(escpos.cutPaper());
...
async function testAlignment() {
    ...
    try {
        await printer.sendData(escpos.init());
        await printer.sendData(escpos.printText('TEST ALIGNMENT', { align: 'center', bold: true }));
        await printer.sendData(escpos.printSeparator('-'));
...
async function testTextSizes() {
    ...
    try {
        await printer.sendData(escpos.init());
        await printer.sendData(escpos.printText('TEST UKURAN TEKS', { align: 'center', bold: true }));
        await printer.sendData(escpos.printSeparator('-'));
...
async function testBarcode() {
    ...
    try {
        await printer.sendData(escpos.init());
        await printer.sendData(escpos.printText('TEST BARCODE', { align: 'center', bold: true }));
        await printer.sendData(escpos.printSeparator('-'));
